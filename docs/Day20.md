...
### Q7. 다음 중 관계(Relationship)의 표기법인 'Crow's Foot' 기호 해석으로 부적절한 것은?
① 기호 'O'가 표시된 쪽은 해당 엔터티의 참여가 필수적임을 의미한다.
② 기호 '<' (까마귀발) 모양은 1:N 관계에서 'Many(다수)' 쪽을 나타낸다.
③ 관계명은 현재형 동사로 기술하며, 능동/수동형 모두 존재할 수 있다.
④ 선택사양(Optionality)에서 원('O') 기호는 매칭되는 데이터가 존재하지 않을 수도 있음을 의미한다.

정답: ①

해설: 'O(원)' 기호는 0(Zero)을 의미하며, 해당 엔터티와의 관계가 **선택적 참여(Optional)**임을 뜻합니다. 필수적 참여(Mandatory)를 나타내는 기호는 직선(|)입니다.

힌트: 기호 'O'를 숫자 '0'이라고 생각하시면 외우기 쉽습니다. "0개여도 괜찮아 = 선택사항!"
